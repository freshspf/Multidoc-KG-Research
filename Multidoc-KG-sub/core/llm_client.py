...
class LLMClient:
    ...
    def __init__(self, model_name: str = "gpt-4o", base_url: Optional[str] = None, timeout: int = 120):
        ...
        self.retry_base_delay = float(os.getenv("LLM_RETRY_BASE_DELAY", "2.0"))
        self.retry_max_delay = float(os.getenv("LLM_RETRY_MAX_DELAY", "12.0"))
        ...
        print(
            f"[LLMClient] Initialized with model: {model_name}, "
            f"base_url: {self.base_url or 'default'}, timeout: {timeout}s, retries: {self.max_retries}"
        )

    def _retry_delay(self, attempt: int) -> float:
        """Return exponential backoff delay for the given retry attempt."""
        delay = self.retry_base_delay * (2 ** max(0, attempt - 1))
        return min(delay, self.retry_max_delay)

    def _is_retryable_exception(self, exc: Exception) -> bool:
        """Best-effort classification of transient errors."""
        if isinstance(exc, (requests.exceptions.Timeout, requests.exceptions.ConnectionError, requests.exceptions.SSLError)):
            return True
    ...
    def generate(self, prompt: str, system_prompt: str = "", json_mode: bool = False, max_tokens: int = 8192) -> str:
        """
        ...
        for attempt in range(1, attempts + 1):
            try:
                if self.use_custom_api:
                    return self._call_custom_api(messages, json_mode, max_tokens)
                ...
                print(f"[LLMClient] Error in generate() [attempt {attempt}/{attempts}]: {str(e)}")

                if not retryable or attempt >= attempts:
                    raise

                delay = self._retry_delay(attempt)
                print(f"[LLMClient] Retrying after {delay:.1f}s due to transient error...")
                time.sleep(delay)

        if last_error:
            raise last_error
        raise RuntimeError("LLM generate failed without a captured exception")
    
    def _call_custom_api(self, messages: List[Dict], json_mode: bool = False, max_tokens: int = 8192) -> str:
        """
        ...
        try:
            # Prepare URL
            # Most OpenAI-compatible providers implement /v1/chat/completions (older) rather than /v1/responses (new).
            url = f"{self.api_scheme}://{self.api_host}/v1/chat/completions"
            ...
            print(f"[LLMClient] Model: {self.model_name}, Messages: {len(messages)}, Timeout: {self.timeout}s, Max Tokens: {max_tokens}")
            ...
            print(f"[LLMClient] SSL error while reaching {self.api_host}")
    ...
    def judge(self, claim: str, context: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.generate(
                ...
                json_mode=True,
                max_tokens=4096  # judge任务不需要太长输出
            )
    ...
    def ground_entities(self, entities: List[str], graph_context: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.generate(
                ...
                system_prompt=system_prompt,
                json_mode=True,
                max_tokens=4096  # grounding任务也不需要太长输出
            )
